...
class Game():
    ...
    def __init__(self, window):
        ...
        self.score = 0
        self.scoreText = pygwidgets.DisplayText(window, (400, 164),
                                   'Player Score: ' + str(self.score),
                                    fontSize=56, textColor=WHITE,
                                    justified='right')
        self.scoreText2 = pygwidgets.DisplayText(window, (400, 110),
                                             'Dealer Score ' + str(self.score),
                                             fontSize=56, textColor=WHITE,
                                             justified='right')
    ...
    def hitHigherOrLower(self, higherOrLower):
        ...
        if higherOrLower == HIGHER:
            if nextCardValue > self.currentCardValue:
                ...
                self.messageText.setValue('You win, dealer busts!')
                self.winnerSound.play()
            else:
                self.score = self.score - Game.POINTS_INCORRECT
                self.messageText.setValue('Dealer wins, you bust!')
        ...
        self.scoreText.setValue('Player Score: ' + str(self.score))
